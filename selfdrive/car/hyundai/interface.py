...
class CarInterface(CarInterfaceBase):
    ...
    @staticmethod
    def get_params(candidate, fingerprint=gen_empty_fingerprint(), has_relay=False,
    ...
    def update(self, c, can_strings):
        ...
        if self.CP.carFingerprint == CAR.KIA_OPTIMA:
            ret.leftBlinker = bool(
                self.CS.left_blinker_flash or self.CS.prev_left_blinker and self.CC.turning_signal_timer)
            ret.rightBlinker = bool(
                self.CS.right_blinker_flash or self.CS.prev_right_blinker and self.CC.turning_signal_timer)

        # turning indicator alert logic
        if (
                ret.leftBlinker or ret.rightBlinker or self.CC.turning_signal_timer) and ret.vEgo < LANE_CHANGE_SPEED_MIN - 1.2:
    ...
    def apply(self, c, sm, LaC):
        can_sends = self.CC.update(c.enabled, self.CS, self.frame, c.actuators,
                                   ...
                                   c.hudControl.rightLaneVisible, c.hudControl.leftLaneDepart,
                                   c.hudControl.rightLaneDepart,
                                   c.hudControl.setSpeed, c.hudControl.leadVisible, sm, LaC)
        self.frame += 1
        return can_sends
